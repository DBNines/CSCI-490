...
# NOTE: These will be recalculated in the main() function before training
GLOBAL_MEAN = -12.9844
GLOBAL_STD = 18.4791

# -----------------------------
# Shared Processing Function (REMAINS THE SAME)
# -----------------------------
...
class UrbanSoundGunshotDataset(Dataset):
    def __init__(self, root_dir, use_folds=None, use_additional_gunshots=False, additional_gunshot_dir=None, ambient_dir=None):
        ...
        metadata_path = os.path.join(root_dir, "UrbanSound8K.csv"); metadata = pd.read_csv(metadata_path)

        for _, row in metadata.iterrows():
            fold = row['fold']; class_name = row['class']
            if use_folds and fold not in use_folds: continue
            filepath = os.path.join(root_dir, f"fold{fold}", row['slice_file_name'])
            self.files.append(filepath); self.labels.append(1 if class_name == "gun_shot" else 0)

        if use_additional_gunshots and additional_gunshot_dir:
            for root, _, filenames in os.walk(additional_gunshot_dir):
                for fname in filenames:
                    if fname.endswith(".wav"):
                        fpath = os.path.join(root, fname); self.files.append(fpath); self.labels.append(1)
        
        # 🚨 Load Ambient Sound/Hard Negatives 🚨
        if ambient_dir and os.path.exists(ambient_dir):
            ...
            for fname in os.listdir(ambient_dir):
                if fname.endswith(".wav"):
                    fpath = os.path.join(ambient_dir, fname); self.files.append(fpath); self.labels.append(0)

    def __len__(self):
        return len(self.files)

    def __getitem__(self, idx):
        filepath = self.files[idx]; label = torch.tensor(self.labels[idx], dtype=torch.long)
        mel = get_spectrogram(filepath, is_training=True) 
        return mel, label

# -----------------------------
# Improved Model (UPDATED FILTERS)
# -----------------------------
class GunshotCNN(nn.Module):
    def __init__(self, n_mels=64, max_len=None):
        super().__init__()
        
        # ⬆️ Block 1: Increased filters from 16 to 32
        self.conv1 = nn.Conv2d(1, 32, 3, padding=1); self.bn1 = nn.BatchNorm2d(32); self.pool1 = nn.MaxPool2d(2) 

        # ⬆️ Block 2: Increased filters from 32 to 64
        self.conv2 = nn.Conv2d(32, 64, 3, padding=1); self.bn2 = nn.BatchNorm2d(64); self.pool2 = nn.MaxPool2d(2)

        # ⬆️ Block 3: Increased filters from 64 to 128
        ...
        # 🚨 Dummy Pass (MUST reflect new filter sizes) 🚨
        with torch.no_grad():
            x = torch.zeros(1, 1, n_mels, max_len)
            x = self.pool1(F.relu(self.bn1(self.conv1(x)))) # 1->32
            x = self.pool2(F.relu(self.bn2(self.conv2(x)))) # 32->64
            x = self.pool3(F.relu(self.bn3(self.conv3(x)))) # 64->128
            self.flattened_size = x.numel()

        self.fc1 = nn.Linear(self.flattened_size, 128); self.fc2 = nn.Linear(128, 2)
        
    def forward(self, x):
        # 🚨 Forward Pass (MUST reflect new filter sizes) 🚨
...
def main():
    ...
    # 2.5. Recalculate Global Normalization Stats
    ...
    GLOBAL_STD = 0.0 # Temporarily disable normalization
    ...
    # 4. Optimizer & Loss 
    # 🚨 RELAXED Class Weights 🚨
    # [Non-Gunshot (0), Gunshot (1)] - Reduced penalty from 10.0 to 3.0
